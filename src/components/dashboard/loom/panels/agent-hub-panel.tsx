...
export function AgentHubPanel({
  ...
  const handleSpawnAgent = () => {
    ...
    toast({ title: "Agent Provisioning Requested", description: `Request to provision agent "${newAgentName}" sent to backend. (Not implemented).` });
    addConsoleMessage('info', `User requested provisioning of new agent: "${newAgentName}". Backend call not implemented.`);
    addTimelineEvent({ type: 'info', message: `Agent provisioning requested for "${newAgentName}".` });
  };

  const handleResumeAll = () => {
    toast({ title: "Agent Hub Action", description: "Request to resume all eligible agents sent to backend. (Not implemented)." });
    addConsoleMessage('info', 'Agent Hub: Request to resume all agents triggered. Backend call not implemented.');
    addTimelineEvent({ type: 'info', message: 'Resume all agents action requested.' });
  };

  const handlePauseAll = () => {
    toast({ title: "Agent Hub Action", description: "Request to pause all active agents (Safe Mode) sent to backend. (Not implemented).", variant: "secondary" });
    addConsoleMessage('warn', 'Agent Hub: Request to pause all agents triggered. Backend call not implemented.');
  ...
  const handleToggleAgentStatus = (agentId: string) => {
    ...
    toast({ title: `Agent Status Change Requested`, description: `Request to ${action} agent "${agentToToggle.name}" sent to backend. (Not implemented).` });
    addConsoleMessage('info', `User requested to ${action} agent "${agentToToggle.name}". Backend call not implemented.`);
  };
    ...
    >
      <div className="space-y-3">
        ...
        <ScrollArea className="h-48 pr-2"> 
          {agents.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-4">No agents currently provisioned. Click "Provision New Agent" to request one.</p>
          ) : (
