  ...
  if (!feedItems || feedItems.length === 0) {
    return (
      <div className="flex items-center justify-center h-full p-4">
        <p className="text-sm text-muted-foreground dark:text-muted-foreground">No feed items available.</p>
      </div>
    );
  }
          ...
          <li key={index} className="p-3 rounded-md bg-card/50 hover:bg-primary/10 dark:bg-card/70 dark:hover:bg-primary/10">
            <div className="flex items-start justify-between mb-1">
              <div className="flex items-center">
                ...
                <div>
                  <p className="font-semibold text-foreground dark:text-foreground text-sm">{item.task}</p>
                  <p className="text-xs text-muted-foreground dark:text-muted-foreground">{item.time}</p>
                </div>
              </div>
              <span className={cn(
