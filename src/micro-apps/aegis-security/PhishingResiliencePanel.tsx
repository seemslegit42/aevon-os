...
import type { PhishingData } from '@/services/security.service';

interface PhishingResiliencePanelProps {
  data: PhishingData;
}
...
const PhishingResiliencePanel: React.FC<PhishingResiliencePanelProps> = ({ data }) => {
  return (
    <Card className="glassmorphism-panel h-full flex flex-col">
      ...
      <CardContent className="flex-grow grid grid-cols-2 gap-4">
        <div className="flex flex-col items-center justify-center">
            <h4 className="text-sm font-semibold text-muted-foreground mb-2">Click Resistance</h4>
             <div className="relative w-full h-32">
                ...
                >
                    <RadialBarChart
                        data={[{ name: 'score', value: data.clickResistance, fill: 'hsl(var(--chart-2))' }]}
                        startAngle={-270}
                        endAngle={90}
                        innerRadius="75%"
                        outerRadius="100%"
        ...
        <div className="flex flex-col items-center">
             ...
             <ChartContainer config={{ count: { label: "Blocked" } }} className="w-full h-32">
                <BarChart data={data.emailsBlocked} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
      ...
      <CardFooter className="justify-center">
        <Badge className={getTrainingColor(data.trainingRequired)}>
          Training Required: {data.trainingRequired}
        </Badge>
      </CardFooter>
    </Card>
  );
